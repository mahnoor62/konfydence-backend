...
router.post('/email-resources', async (req, res) => {
  try {
    ...
    if (!bundleKey || !bundleDownloads[bundleKey])
      return res.status(400).json({ success: false, message: 'Invalid bundle' });

    if (!email || !emailRegex.test(email))
      return res.status(400).json({ success: false, message: 'Invalid email' });

    // IMPORTANT: use FRONTEND_URL to generate links (where PDFs actually exist)
    ...
    const files = bundleDownloads[bundleKey]; // use mapping directly

    const html = `
      <html>
        <body style="font-family: Arial, sans-serif; color:#222;">
          <p>Hello,</p>
          <p>As requested, here are your Konfydence resources:</p>
          <ul>
            ${files.map((f) => {
              const url = `${frontendUrl}/pdfs/${encodeURI(f)}`;
              const name = f.split('/').pop();
    ...
    const mailOptions = {
      ...
      subject: 'Your requested Konfydence resources',
      html,
      text: files.map((f) => `${frontendUrl}/pdfs/${f}`).join('\n'),
      // attachments removed (important)
    };

    const info = await transporter.sendMail(mailOptions);

    return res.status(200).json({ success: true, messageId: info.messageId, files });
  } catch (err) {
    console.error('Error in email-resources route:', err);
    return res.status(500).json({ success: false, message: err.message || 'Failed to send email' });
  }
